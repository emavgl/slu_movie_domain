...
ngramcount --order=9 --require_symbols=false concepts.far > concepts.cnts

ngrammake --method=kneser_ney concepts.cnts > trlm.lm
...
for f in ./fst/*
do
    echo "Processing $f file using 9-gram and kneser_ney"
...
awk '{print $4}' results.txt | paste -d '\t' data/NLSPARQL.test.data - | sed -r 's/(O-(.)*)/O/' > to_evaluate.txt
